...
import com.ap.moviemaster.databinding.WidgetMovieBinding
...
class 	MovieWidget @JvmOverloads constructor(
	...
	init {
		...
		context.obtainStyledAttributes(attrs, R.styleable.MovieWidget).run {
			val drawable = getDrawable(R.styleable.MovieWidget_image)
			setMovieImage(drawable ?: ColorDrawable(Color.WHITE))

			setMovieImageAlpha(getFloat(R.styleable.MovieWidget_imageAlpha, 1.0F))
			setMovieImageElevationAlpha(getFloat(R.styleable.MovieWidget_imageElevationAlpha, 1.0F))
			setMovieImageTopFadeAlpha(getFloat(R.styleable.MovieWidget_imageTopFadeAlpha, 0.0F))
			setMovieImageZoom(getFloat(R.styleable.MovieWidget_imageZoom, 1.0F))
			setMovieImagePanY(getFloat(R.styleable.MovieWidget_imagePanY, 0.0F))
